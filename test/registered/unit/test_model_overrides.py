...
class TestGoldenModelOverrides(_IsolatedPublish):
    ...
    def test_qwen4_ple_offload_default(self):
        ...
        # Platform facts live at one address now; the provider asks
        # get_platform().is_cuda, so the test overrides the fact instead of
        # patching a per-module import.
        with override_platform(is_cuda=True):
            for kwargs, expected in (
                ...
                with self.subTest(kwargs=kwargs):
                    sa = self._construct(*qwen4, **kwargs)
                    self.assertEqual(
                        self._resolved(sa, "ple_offload_embedding"),
                        expected,
                    )
        ...
        with override_platform(is_cuda=False):
            ...
            # no_buffer so this case isolates the PLE default, not the
            # platform probe (no_buffer also needs the overlap scheduler
            # disabled).
            sa = self._construct(
                *qwen4,
            ...
            self.assertFalse(self._resolved(sa, "ple_offload_embedding"))
