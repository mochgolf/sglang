...
def _sm89_sparse_mla_prefill_kernel(
    ...
    tl.store(
        out + row_id * out_stride_0 + head_id * out_stride_1 + offs_v * out_stride_2,
        out_vals,
        mask=v_mask,
    )
...
def select_sm89_sparse_mla_block_m(block_m: int | None = None) -> int:
    ...
    try:
        ...
        raise ValueError(
            "SGLANG_GLM_DSA_SM89_BLOCK_M must be one of 1, 2, or 4; " f"got {raw!r}."
        ) from exc
    if selected not in {1, 2, 4}:
        raise ValueError(
            "SGLANG_GLM_DSA_SM89_BLOCK_M must be one of 1, 2, or 4; " f"got {selected}."
        )
    return selected
...
def select_sm89_sparse_mla_split_k(split_k: int | None = None) -> int:
    ...
    try:
        ...
        raise ValueError(
            "SGLANG_GLM_DSA_SM89_SPLIT_K must be one of 1, 4, or 8; " f"got {raw!r}."
        ) from exc
    if selected not in {1, 4, 8}:
        raise ValueError(
            "SGLANG_GLM_DSA_SM89_SPLIT_K must be one of 1, 4, or 8; " f"got {selected}."
        )
    return selected
