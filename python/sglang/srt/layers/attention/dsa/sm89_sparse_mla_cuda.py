...
def select_sm89_sparse_mla_decode_kernel(value: str | None = None) -> str:
    selected = (
        ...
        else os.environ.get(SM89_DSA_DECODE_KERNEL_ENV, "three_stage")
    )
...
def splitk_workspace_schema(
    ...
    if split_tokens not in (32, 64) or topk % split_tokens:
        raise ValueError(
            "split-K workspace requires split_tokens in {32,64} dividing topk"
        )
...
def select_sm89_sparse_mla_cuda_impl(cuda_impl: str | None = None) -> str:
    selected = cuda_impl or os.environ.get("SGLANG_GLM_DSA_SM89_CUDA_IMPL", "simt")
    selected = selected.strip().lower()
    if selected not in ("simt", "tensorcore"):
        raise ValueError("SGLANG_GLM_DSA_SM89_CUDA_IMPL must be 'simt' or 'tensorcore'")
    return selected
...
def sm89_sparse_mla_prefill_cuda(
    ...
    if block_n not in (None, 32):
        raise ValueError(
            "sm89_sparse_mla_prefill_cuda currently accepts only block_n=32."
        )
...
def _validate_sm89_sparse_mla_decode_cuda_splitk(
    ...
    try:
        effective_sm_scale = struct.unpack("=f", struct.pack("=f", input_sm_scale))[0]
